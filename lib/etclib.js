...
etclib.isValid = function (token) {
    ...
    _data.read('actk', token, function (err, data) {
        if (!err && data) {
            ...
            const valid = data.expires > Math.floor(Date.now() / 1000);
            return valid;
        } else {
            return false;
        }
...
etclib.grants = function (token, userEmail) {
    //Callback: error, validity
    //Check if a token is time valid
    try {
        const tmp = etclib.isValid(token);
        ...
        _data.read('actk', token, function (err, tokdat) {
            if (!err) {
                console.log(tokdat.email + "  .  " + userEmail);
                console.log(tmp);
