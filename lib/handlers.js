...
handlers.accesstoken.post = function (data, callback) {
    ...
    const pass = typeof data.payload.pass === 'string' && data.payload.st(data.payload.pass) === true ? data.payload.pass : false;

    if (email && pass) {
        _data.read('users', email, function (err, udata) {
            const sentPassHash = etc.hash(pass + udata.salt);
            if (udata.hashedPass === sentPassHash && email === udata.email) {
                const token = etc.newToken();//Users token
                const expiry = Date.now() + 100 * 60 * config.tokenTime;//Time until token expiry
