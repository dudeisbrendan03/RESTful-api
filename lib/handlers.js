    ...
    if (email && pass) {
        _data.read('users', email, function (err, udata) {
            ...
            if (udata.pass === sentPassHash && email === udata.email) {
                ...
                const expires = Date.now() + 1000 * 60 * config.tokenTime;//Time until token expiry
...
handlers.accesstoken.get = function (data, callback) {
    ...
    if (token) {
        ...
        callback(404, { status: 404, error: "TOKEN-NOEXIST", desc: "The token in the query does not exist." });
    }
};
...
handlers.accesstoken.put = function (data, callback) {
    const token = typeof data.queryStringObj.token === 'string' && data.queryStringObj.token.trim().length === config.tokenLength ? data.queryStringObj.email : false;//get the users email

    _data.read('actk', token, function (err, data) {
        if (!err) {
            data.expires = Date.now() + 1000 * 60 * config.tokenTime;
            ...
            callback(404, { status: 404, error: "TOKEN-NOEXIST", desc: "The token in the payload does not exist." });
        }
    });
};
 ...
 *
 */
handlers.accesstoken.delete = function (err, data) {
    const token = typeof data.queryStringObj.token === 'string' && data.queryStringObj.token.trim().length === config.tokenLength ? data.queryStringObj.email : false;//get the users email
