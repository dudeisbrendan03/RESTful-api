...
handlers.user = function (data, callback) {
    //if (['post', 'get', 'put', 'delete'].indexOf(data.method)) {
...
handlers.user.post = function (data, callback) {
    ...
    const mobile = typeof data.payload.mobile === 'string' && data.payload.mobile.trim().length === 11 ? data.payload.mobile.trim() : false;// same with last name ^^
