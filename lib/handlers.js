                ...
                if (!err && data.op) {
                    ...
                    _data.update('actk', token, tokenStore, function(err) {
                        if (!err) {
                            ...
                            callback(500, { status: 500, error: "", desc: "An unclassified error prevented the token from being modified. This error is vague due to the fact that this feature is in development. Check that the token is correct and that you have the privileges to perform this operation." });
                        }
                    })
                } else {
