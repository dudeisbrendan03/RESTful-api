...
handlers.user.post = function (data, callback) {
    const format = /[ !@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/,
        phoneformat = /^[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*$/g,
        fName = typeof data.payload.fName === 'string' && data.payload.fName.trim().length > 0 && (!phoneformat.test(data.payload.fName)) && (!format.test(data.payload.fName))? data.payload.fName.trim() : false, //get first name, if invalid or doesn't exist default to false
        lName = typeof data.payload.lName === 'string' && (!phoneformat.test(data.payload.lName)) &&data.payload.lName.trim().length > 0 && (!format.test(data.payload.lName))? data.payload.lName.trim() : false,// same with last name ^^
        email = typeof data.payload.email === 'string' && data.payload.email.trim().length > 2 && data.payload.email.includes('@') ? data.payload.email : false,
        mobile = typeof data.payload.mobile === 'string'  && phoneformat.test(data.payload.mobile) && data.payload.mobile.trim().length >= 11 && data.payload.mobile.trim().length <= 21 ? data.payload.mobile.trim() : false,// same with last name ^^
        pass = typeof data.payload.pass === 'string' && data.payload.pass.length > 8 && format.test(data.payload.pass) === true ? data.payload.pass : false,
        tos = typeof data.payload.tos === 'boolean' && data.payload.tos === true ? true : false;// same with last name ^^
...
handlers.accesstoken.get = function (data, callback) {
    ...
    if (token) {
        try {
            etc.isValid(String(token), function (valid) {
                if (token && valid) {
