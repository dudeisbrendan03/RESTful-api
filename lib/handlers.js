        ...
        try {
            etc.grants({ "token":String(token), email }, function (valid) {
                if (valid) {
                    ...
                    callback(401, { status: 401, error: "AUTH-MATCHTOKEN", desc: "The token could not be matched with the user when on call." });
                }
            });
        } catch (e) {
            callback(500, { status: 500, error: "", desc: "An unknown error has occured." });
        ...
        try {
            etc.isValid(token, function (valid) {
                if (email && valid) {
                    ...
                    if (valid) { callback(401, { status: 401, error: "USER-NO-EXIST", desc: "The user in the query does not exist." }); }
